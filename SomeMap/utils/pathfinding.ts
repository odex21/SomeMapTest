...
const checkObstacle = (x1: number, y1: number, x2: number, y2: number, grid: Grid): ArrayPoint => {
    ...
    if (!fly && !holeType && isNotNeedFind) {
      ...
      while ((dx + dy > 1)) {
        ...
        if (tempCol > -1 && tempRow > -1) {
          ...
          const getTempSection = (arr: ArrayPoint[]) => {
            if (arr.length > 1) return arr.slice(1)
              ...
              return last.slice(1) as ArrayPoint[]
            }
          }
          tempSection = getTempSection(tempSection);
